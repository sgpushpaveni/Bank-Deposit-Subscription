...
if not os.path.exists(model_path + r"/XGBoost.joblib"):
    raise FileNotFoundError(f"Model file not found at {model_path}")
else:
    model = joblib.load(model_path + r"/XGBoost.joblib")
    preprocessor = joblib.load(model_path + r"/preprocessor.joblib")
    ...
    with st.sidebar:
        opt_selected = option_menu(
            ...
            options=["File Upload", "Single Data"],
            icons=["list", "gear" ],
            menu_icon="cast",
            default_index=0,
        )
